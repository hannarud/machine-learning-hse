...
# Кстати, задача поставлена лайтово. Фамилии в основном различаются, поэтому можно и их тоже разобрать, но потом в процессе поиска частот они сами выпадут, потому что фамилии встречаются реже, чем имена. Этим ограничимся.
# Для незамужних дам нужно отбросить фамилию и обращение Miss. (то есть ищем по регулярному выражению)
titanic_women_names1 = titanic_women_names.str.replace(r'[^,]*, Miss\. ', '')

# Для замужних дам нас интересует их девичье имя, которое будет написано в скобках, так и ищем
# Пока для начала можно сделать по-школярски - удалить сначала все до скобок, а потом все после закрывающейся скобки
titanic_women_names1 = titanic_women_names1.str.replace(r'[^\(]*\(', '')
titanic_women_names1 = titanic_women_names1.str.replace(r'\).*', '')
# Но вообще по-хорошему нужно извлечь текст внутри скобочек. Оставим это на потом
# titanic_women_names1 = [re.search(r'[^\(]*\(([^\)]*)\)', i).group(1) for i in titanic_women_names1]
# titanic_women_names1 = titanic_women_names.str.replace(r'[^\(]*\(([^\)]*)\)', '\1')

# После этого нужно разбить весь этот вектор на слова при помощи пробела. И подсчитать количество.
titanic_women_names1 = [j for i in titanic_women_names1.tolist() for j in i.split()]

from collections import Counter
word_counts = Counter(titanic_women_names1)

# Выбираем самый частовстречающийся элемент, его "ключ"
resulting_string = word_counts.most_common(1)[0][0]
